...
for i in range(9):
    ...
    metric[i] = kmeans.inertia_/len(X)
    silhouette_kmeans[i] = silhouette_score(X,\
                                     kmeans.labels_, metric=dist_type)
    if i+2==3:
        fig, axs = plt.subplots(2,2,figsize=(25, 25))
        fig.suptitle("K-means (3 Clusters)", fontsize=50, y=0.93)
        ##
        axs[0,0].scatter(np.transpose(X)[0],\
                np.transpose(X)[1],\
                    c=kmeans.labels_,alpha = 1,s=250)
        axs[0,0].scatter(np.transpose(kmeans.cluster_centers_)[0],\
                              np.transpose(kmeans.cluster_centers_)[1],\
                                  c='b',marker='x',s=250)
        axs[0,0].set_xlabel("sepal.length", fontsize=40)
        axs[0,0].set_ylabel("sepal.width", fontsize=40)
        ##
        axs[0,1].scatter(np.transpose(X)[0],\
                np.transpose(X)[2],\
                    c=kmeans.labels_,alpha = 1,s=250)
        axs[0,1].scatter(np.transpose(kmeans.cluster_centers_)[0],\
                              np.transpose(kmeans.cluster_centers_)[2],\
                                  c='b',marker='x',s=250)
        axs[0,1].set_xlabel("sepal.length", fontsize=40)
        axs[0,1].set_ylabel("petal.length", fontsize=40)
        ##
        axs[1,0].scatter(np.transpose(X)[0],\
                np.transpose(X)[3],\
                    c=kmeans.labels_,alpha = 1,s=250)
        axs[1,0].scatter(np.transpose(kmeans.cluster_centers_)[0],\
                              np.transpose(kmeans.cluster_centers_)[3],\
                                  c='b',marker='x',s=250)
        axs[1,0].set_xlabel("sepal.length", fontsize=40)
        axs[1,0].set_ylabel("petal.width", fontsize=40)
        ##
        axs[1,1].scatter(np.transpose(X)[2],\
                np.transpose(X)[3],\
                    c=kmeans.labels_,alpha = 1,s=250)
        axs[1,1].scatter(np.transpose(kmeans.cluster_centers_)[2],\
                              np.transpose(kmeans.cluster_centers_)[3],\
                                  c='b',marker='x',s=250)
        axs[1,1].set_xlabel("petal.length", fontsize=40)
        axs[1,1].set_ylabel("petal.width", fontsize=40)
plt.figure()
plt.plot(K,metric)
plt.xlabel("Number of Clusters, K")
plt.ylabel("Metric")
plt.title("K-means of Iris Data")
plt.figure()
...
threshold = np.array([1,1.5,1.9,3])  # case without normalization
#threshold = np.array([1,1.8,2.9,3.5])  # case with normalization
fig, axs = plt.subplots(2,2,figsize=(25, 25))
fig.suptitle("Agglomerative Clustering \n (linkage: average, distance_threshold, sepal.length v. sepal.width)"\
             ,fontsize=50, y=0.96)
...
for i in range(4):
    agglomerative= AgglomerativeClustering(n_clusters = None,\
                                           ...
                                           affinity=dist_type)\
                                            .fit(X)
    silhouette_agglomerative_threshold[i] = silhouette_score(X,\
                                     agglomerative.labels_, metric=dist_type)
    axs[i//2,i%2].scatter(np.transpose(X)[0],\
            np.transpose(X)[1],\
                c=agglomerative.labels_,s=250)
...
fig.suptitle("Agglomerative Clustering \n (linkage: average, sepal.length v. sepal.width)", \
             fontsize=50, y=0.96)
...
for i in range(9):
    agglomerative= AgglomerativeClustering(n_clusters=i+2,linkage='average',\
                                           affinity=dist_type)\
                                            .fit(X)
    silhouette_agglomerative_average[i] = silhouette_score(X,\
                                     agglomerative.labels_, metric=dist_type)
    axs[i//3,i%3].scatter(np.transpose(X)[0],\
            np.transpose(X)[1],\
                c=agglomerative.labels_,s=250)
    axs[i//3 ,i%3].set_title(f"number of clusters: {i+2}")
dendrogram=hierarchy.linkage(X,method='average',metric=dist_type)
plt.figure()
dn = hierarchy.dendrogram(dendrogram)
plt.title("Dendrogram of Iris Data (linkage: average)")

###################################

# linkage: single
fig, axs = plt.subplots(3,3,figsize=(25, 25))
fig.suptitle("Agglomerative Clustering \n (linkage: single, sepal.length v. sepal.width)", \
             fontsize=50, y=0.96)
...
for i in range(9):
    agglomerative= AgglomerativeClustering(n_clusters=i+2,linkage='single',\
                                           affinity=dist_type)\
                                            .fit(X)
    silhouette_agglomerative_single[i] = silhouette_score(X,\
                                     agglomerative.labels_, metric=dist_type)
    axs[i//3,i%3].scatter(np.transpose(X)[0],\
            np.transpose(X)[1],\
                c=agglomerative.labels_,s=250)
    axs[i//3 ,i%3].set_title(f"number of clusters: {i+2}")
dendrogram =hierarchy.linkage(X,method='single',metric=dist_type)
plt.figure()
dn = hierarchy.dendrogram(dendrogram)
plt.title("Dendrogram of Iris Data (linkage: single)")

###################################

# linkage: ward (only with euclidean distance)
fig, axs = plt.subplots(3,3,figsize=(25, 25))
fig.suptitle("Agglomerative Clustering \n (linkage: ward, sepal.length v. sepal.width)", \
             fontsize=50, y=0.96)
...
for i in range(9):
    agglomerative= AgglomerativeClustering(n_clusters=i+2,linkage='ward',\
                                           affinity='euclidean')\
                                            .fit(X)
    silhouette_agglomerative_ward[i] = silhouette_score(X,\
                                     agglomerative.labels_, metric='euclidean')
    axs[i//3,i%3].scatter(np.transpose(X)[0],\
            np.transpose(X)[1],\
                c=agglomerative.labels_,s=250)
    axs[i//3 ,i%3].set_title(f"number of clusters: {i+2}")
dendrogram =hierarchy.linkage(X,method='ward',metric='euclidean')
plt.figure()
dn = hierarchy.dendrogram(dendrogram)
plt.title("Dendrogram of Iris Data (linkage: ward)")
...
plt.ylabel("Silhouette Score")
plt.title("Agglomerative Clustering of Iris Data (linkage: average)")
...
plt.xlabel("Number of Clusters, K")
plt.ylabel("Silhouette Score")
plt.title("Agglomerative Clustering of Iris Data")
...
## DBSCAN
fig, axs = plt.subplots(3,3,figsize=(25, 25))
fig.suptitle("DBSCAN Clustering (sepal.length v. sepal.width)", fontsize=50, y=0.93)
EPS = np.linspace(0.5, 1, num=3)
Min_samples = np.linspace(5, 20, num=3)
silhouette_dbscan = np.zeros(15,'d')-2
count = 0
for i in range(3):
    for j in range(3):
        dbscan = DBSCAN(eps=EPS[i], min_samples=Min_samples[j], \
                        metric=dist_type).fit(X)
        axs[j ,i%3].scatter(np.transpose(X)[0],\
                    np.transpose(X)[1],\
                        c=dbscan.labels_,s=250)
        axs[j ,i%3].title.set_text(\
                            f"eps: {EPS[i]}; min_samples: {Min_samples[j]}")
        if np.sum(dbscan.labels_)>0:
            silhouette_dbscan[count] = silhouette_score(X,\
...
##
fig, axs = plt.subplots(3,3,figsize=(25, 25))
fig.suptitle("DBSCAN Clustering (petal.length v. petal.width)", fontsize=50, y=0.93)
for i in range(3):
    for j in range(3):
        dbscan = DBSCAN(eps=EPS[i], min_samples=Min_samples[j], \
                        metric=dist_type).fit(X)
        axs[j ,i%3].scatter(np.transpose(X)[2],\
                    np.transpose(X)[3],\
                        c=dbscan.labels_,s=250)
        axs[j ,i%3].title.set_text(\
                            f"eps: {EPS[i]}; min_samples: {Min_samples[j]}")
...
for i in range(9):
    gm_full = GaussianMixture(n_components=i+2).\
        fit(X)
    #plot_contours(X, gm_full.means_, gm_full.covariances_,\
    #              "GMM (covariance_type: full)")
    gm_full_labels = gm_full.fit_predict(X)
    silhouette_gm_full[i] = silhouette_score(X,\
...
for i in range(9):
    gm_tied = GaussianMixture(n_components=i+2,covariance_type='tied').\
        fit(X)
    #plot_contours(X, gm_tied.means_, gm_tied.covariances_,\
    #              "GMM (covariance_type: tied)")
    gm_tied_labels = gm_tied.fit_predict(X)
    silhouette_gm_tied[i] = silhouette_score(X,\
...
for i in range(9):
    gm_diag = GaussianMixture(n_components=i+2,covariance_type='diag').\
        fit(X)
    plot_contours(X, gm_diag.means_, gm_diag.covariances_,\
                  "GMM (covariance_type: diag)")
    gm_diag_labels = gm_diag.fit_predict(X)
    silhouette_gm_diag[i] = silhouette_score(X,\
...
for i in range(9):
    gm_spherical = GaussianMixture(n_components=i+2,covariance_type\
                                   ='spherical').fit(X)
    plot_contours(X, gm_spherical.means_, gm_spherical.covariances_,\
                  "GMM (covariance_type: spherical)")
    gm_spherical_labels = gm_spherical.fit_predict(X)
    silhouette_gm_spherical[i] = silhouette_score(X,\
